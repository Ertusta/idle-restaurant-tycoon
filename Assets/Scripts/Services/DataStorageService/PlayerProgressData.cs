...
namespace Services.DataStorageService
{
    ...
    public class PlayerProgressData
    {
        ...
        public PlayerProgressData()
        {
            ...
            {
                new("Yemek", $"Fiyatlar yükselir {PriceIncreaseRate}%", new List<int> { 1000, 2000, 3000, 5000 }, () =>
                    {
                        RaisePrices();
                        Meals.UpdatePriceMultiplier();
                    }),
                
                new("Musteri", $"hýzlý yemek yer {2f}sec", new List<int> { 1500, 2500, 4500, 6500, 8000 }, () =>
                {
                    ReduceTime(ref Customers.EatingTimeDelay, 2f);
                    Customers.UpdateEatingTimeDelay();
                }),
                new("sef", $"Hýzlý yemek hazýrlar {2f}sec", new List<int> { 2000, 2500, 3000 }, () =>
                {
                    ReduceTime(ref Staff.Chef.FoodSearchingTimeDelay, 2f);
                    Staff.Chef.UpdateFoodSearchingTime();
                }),
                new("sef", $"Hýzlý yemek piþirir {2f}sec", new List<int> { 2500, 3000, 3500 }, () =>
                {
                    ReduceTime(ref Staff.Chef.CookingTimeDelay, 2f);
                    Staff.Chef.UpdateCookingTime();
                }),
                new("sef", $"Increase speed by {SpeedIncreaseRatePercent}%", new List<int> { 500, 750, 1000, 1250, 1500 }, () =>
                {
                    IncreaseSpeed(ref Staff.Chef.Speed);
                    Staff.Chef.UpdateSpeed();
                }),
                new("Garson", $"Hýzlanýr {SpeedIncreaseRatePercent}%", new List<int> { 500, 750, 1000, 1250, 1500 }, () =>
                {
